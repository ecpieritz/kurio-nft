...
import {
  lighthouseAuditConfig as config,
...
const enforceThresholds =
  process.env
    .LIGHTHOUSE_ENFORCE_THRESHOLDS ===
  'true'

const shouldStartPreview =
  /^https?:\/\/(?:127\.0\.0\.1|localhost):4173\/?$/u.test(
    baseUrl,
  )
...
async function waitForServer(
  ...
  throw new Error(
    `Preview did not become available at ${url} within ${timeoutMs}ms.`,
  )
}
...
async function main() {
  ...
  try {
    if (
      ...
      console.log(
        `[Lighthouse] Iniciando preview em ${baseUrl}`,
      )
    ...
    const summary = {
      ...
      environment: {
        ...
        lighthouse:
          config.lighthouseVersion,

        scenario:
          config.scenario,

        runsPerPageAndProfile:
          config.runs,
      },
