...
function RoutedApplication() {
  ...
  useEffect(() => {
    if (!auth.sessionExpiredAt || handledExpiration.current === auth.sessionExpiredAt) return
    ...
    void router.navigate({
      to: paths.login,
      search: { redirect: returnTo, reason: 'session-expired' },
      replace: true,
    })
  }, [auth.sessionExpiredAt])

  useEffect(() => {
    if (!auth.loginCompletedAt || handledLogin.current === auth.loginCompletedAt) return
