...
function assertDecimalString(
  ...
  if (!decimalPattern.test(value)) {
    throw new Error(
      `Invalid ETH decimal value: ${value}`,
    )
  }
}
...
export function subtractEth(
  ...
  if (result < 0n) {
    throw new Error(
      'ETH subtraction cannot produce a negative value.',
    )
  }
...
export function multiplyEth(
  ...
  if (
    ...
    throw new Error(
      'ETH multiplier must be a non-negative safe integer.',
    )
  }
...
export function percentageOfEth(
  value: DecimalString,
  percentage: number,
): DecimalString {
  if (
