...
import { buildQuoteRequest, hasQuoteChanged } from '@/features/checkout/checkout-quote'
...
function getQuoteErrorMessage(error: Error | null): string | null {
  if (!error) {
    return null
  }
...
function getOrderErrorMessage(error: Error | null): string | null {
  if (!error) {
    return null
  }

  if (error instanceof ApiClientError) {
    if (error.code === 'AVAILABILITY_CONFLICT' || error.code === 'CONFLICT') {
...
function validateCollector(collector: CollectorCheckoutDetails): string | null {
  if (
...
function CheckoutContent({ cart, wallets, user }: CheckoutContentProps) {
  const navigate = useNavigate()

  const createOrderMutation = useCreateOrderMutation()

  const primaryWallet = wallets.items.find((wallet) => wallet.primary) ?? wallets.items[0] ?? null
  ...
  const [collector, setCollector] = useState<CollectorCheckoutDetails>(() => ({
    ...
    profileName: primaryWallet?.profileName ?? '',

    ensName: primaryWallet?.ensName ?? '',
  ...
  const [pendingRecoveryKey, setPendingRecoveryKey] = useState<string | null>(
    () => readPendingOrderRecovery(user.id)?.idempotencyKey ?? null,
  )

  const idempotencyKeyRef = useRef<string | null>(pendingRecoveryKey)

  const submissionLockRef = useRef(false)

  const recoveryQuery = usePendingOrderRecoveryQuery(pendingRecoveryKey)

  const quoteSignature = selectedWallet
    ? JSON.stringify({
        ...
        items: cart.items.map((item) => ({
          ...
          availabilityChanged: item.availabilityChanged,
        })),
        ...
        refresh: quoteRefreshNonce,
      })
    : null

  useEffect(() => {
    if (!selectedWallet || cart.items.length === 0 || !quoteSignature) {
      return
    }

    let active = true

    void buildQuoteRequest(cart, selectedWallet, appliedCoupon)
      .then((request) => {
        if (!active) {
          return null
        }
      ...
      .catch((error: unknown) => {
        ...
        setQuoteError(error instanceof Error ? error : new Error('Quote request failed.'))
  ...
  }, [appliedCoupon, cart, quoteSignature, selectedWallet])

  useEffect(() => {
    const recoveredOrder = recoveryQuery.data
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (pendingRecoveryKey && recoveryQuery.isFetching) {
      return
    }

    if (!selectedWallet) {
      setFormError('Selecione uma carteira antes de confirmar a compra.')

      return
    }

    const collectorError = validateCollector(collector)

    if (collectorError) {
      setFormError(collectorError)
    ...
    try {
      const freshRequest = await buildQuoteRequest(cart, selectedWallet, appliedCoupon)

      const freshQuote = await createQuote(freshRequest)

      if (!quote) {
        setQuote(freshQuote)

        setReviewNotice('A cotação foi atualizada. Revise os valores antes de confirmar a compra.')
      ...
      persistPendingOrderRecovery(user.id, idempotencyKey)

      setPendingRecoveryKey(idempotencyKey)
      ...
      const order = await createOrderMutation.mutateAsync({
        ...
        request: {
          ...
          collector: {
            ...
            profileName: collector.profileName.trim(),

            ensName: collector.ensName.trim(),

            note: collector.note?.trim() || undefined,
          },
        },
      })
      ...
      if (uncertainOutcome && idempotencyKeyRef.current) {
        try {
          const recoveredOrder = await recoverOrder(idempotencyKeyRef.current)
      ...
      if (error instanceof Error && !createOrderMutation.error) {
        setQuoteError(error)
      }
    } finally {
  ...
  const quoteErrorMessage = getQuoteErrorMessage(quoteError)

  const orderErrorMessage = getOrderErrorMessage(createOrderMutation.error)

  const hasAvailabilityConflict = cart.items.some(
  ...
  return (
    <form onSubmit={handleFormSubmit}>
      ...
      <div className="mb-7 flex items-center gap-4 md:hidden">
      ...
      <div className="grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_23rem]">
        ...
        <aside className="min-w-0 max-w-full overflow-hidden rounded-panel border-0 bg-transparent p-0 md:border md:border-border/70 md:bg-card md:p-5 lg:sticky lg:top-24">
          ...
          <div className="mt-5 hidden space-y-3 md:block">
            {cart.items.map((item) => {
              const quoteItem = quote?.items.find((candidate) => candidate.cartItemId === item.id)

              return (
                <div key={item.id} className="grid grid-cols-[3.5rem_minmax(0,1fr)_auto] gap-3">
                  <img
                    src={item.image.url}
                    alt={item.image.alt}
                  ...
                  <span className="text-sm font-bold text-primary">
                    {quoteItem?.subtotalEth ?? item.unitPriceEth} ETH
                  </span>
                </div>
              )
            })}
          ...
          <div className="mt-5 hidden border-t border-border/70 pt-5 md:block">
            <label htmlFor="checkout-coupon" className="text-xs font-semibold">
              Código promocional
            </label>
            ...
            <div className="mt-2 flex gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setAppliedCoupon(normalized || undefined)
            ...
            {quote?.coupon.status === 'applied' && (
              <p className="mt-2 text-xs text-primary">Cupom {quote.coupon.code} aplicado.</p>
            )}

            {quote?.coupon.status === 'invalid' && (
              <p className="mt-2 text-xs text-destructive">Código promocional inválido.</p>
            )}

            {quote?.coupon.status === 'expired' && (
              <p className="mt-2 text-xs text-destructive">Este código promocional expirou.</p>
            )}
          </div>
          ...
          <dl className="mt-5 space-y-3 border-t border-border/70 pt-5 text-sm">
            <div className="hidden justify-between gap-4 md:flex">
              ...
              <dd>{quote?.subtotalEth ?? '—'} ETH</dd>
            </div>

            <div className="hidden justify-between gap-4 md:flex">
              <dt>Desconto do lançamento</dt>

              <dd>(-) {quote?.discountEth ?? '—'} ETH</dd>
            </div>

            <div className="hidden justify-between gap-4 md:flex">
              <dt>Taxa de rede</dt>

              <dd>{quote?.networkFeeEth ?? '—'} ETH</dd>
            </div>

            <div className="flex justify-between gap-4 border-t border-border/70 pt-4 font-bold">
              <dt>Total</dt>

              <dd className="text-primary">{quote?.totalEth ?? '—'} ETH</dd>
            </div>
          </dl>
          ...
          <div className="mt-6">
            ...
            {wallets.items.length === 0 ? (
              <div className="mt-3 rounded-control border border-dashed p-4 text-sm text-muted-foreground">
                ...
                <Link to="/wallets" className="font-semibold text-primary">
                  Adicionar carteira
                </Link>
              </div>
            ) : (
          ...
          {quoteLoading && (
            <p className="mt-4 text-xs text-muted-foreground">Atualizando cotação...</p>
          )}

          {pendingRecoveryKey && recoveryQuery.isFetching && (
...
export function CheckoutPage() {
  ...
  if (cartQuery.isPending || walletsQuery.isPending || !auth.user) {
    return (
      <main
        id="main-content"
        tabIndex={-1}
      ...
      >
        <div role="status" aria-label="Carregando pagamento" className="space-y-5">
  ...
  if (cartQuery.isError || walletsQuery.isError || !cartQuery.data || !walletsQuery.data) {
    return (
      <main
        id="main-content"
        tabIndex={-1}
      ...
      >
        <section role="alert" className="max-w-xl rounded-panel border bg-card p-8 text-center">
          <Typography as="h1" variant="heading">
  ...
  return (
    ...
    >
      <CheckoutContent cart={cartQuery.data} wallets={walletsQuery.data} user={auth.user} />
    </main>
  )
}
