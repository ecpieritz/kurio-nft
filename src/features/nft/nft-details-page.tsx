...
function DetailsSkeleton() {
  return (
    <div role="status" aria-label="Carregando detalhes do NFT" className="grid gap-8 lg:grid-cols-2">
...
export function NftDetailsPage() {
  const { nftId } = useParams({ from: '/_public/nfts/$nftId' })
  const nftQuery = useNftDetailsQuery(nftId)
  const relatedQuery = useCatalogQuery({ sort: 'popular', page: 1, pageSize: 6 })
  ...
  if (nftQuery.isPending) {
    return (
      <main id="main-content" tabIndex={-1} className="mx-auto max-w-(--content-max) px-(--page-gutter) py-8 md:py-12">
        <DetailsSkeleton />
      </main>
    )
  }
  ...
  if (nftQuery.error instanceof ApiClientError && nftQuery.error.status === 404) {
    return (
      <main id="main-content" tabIndex={-1} className="grid min-h-[65svh] place-items-center px-(--page-gutter)">
  ...
  if (nftQuery.isError || !nftQuery.data) {
    return (
      <main id="main-content" tabIndex={-1} className="grid min-h-[65svh] place-items-center px-(--page-gutter)">
        <section role="alert" className="max-w-xl rounded-panel border border-destructive/60 bg-card p-8 text-center">
          <Typography as="h1" variant="heading">
            N\u00e3o foi poss\u00edvel carregar este NFT
          </Typography>
          <Button type="button" variant="outline" className="mt-6" onClick={() => void nftQuery.refetch()}>
            Tentar novamente
          </Button>
        </section>
      </main>
  ...
  const selectedEdition =
    ...
    nft.editions[0]
  const selectedImage = nft.gallery[galleryIndex] ?? nft.image
  const maxQuantity = selectedEdition?.availableQuantity ?? 0
  const relatedNfts = relatedQuery.data?.items.filter((item) => item.id !== nft.id).slice(0, 5) ?? []

  return (
    <main id="main-content" tabIndex={-1} className="mx-auto w-full max-w-(--content-max) px-(--page-gutter) py-6 md:py-10">
      <nav aria-label="Breadcrumb" className="mb-5 hidden text-sm md:block">
        <Link to="/" className="hover:text-primary">In\u00edcio</Link>
        <span aria-hidden="true"> / </span>
        <Link to="/marketplace" className="hover:text-primary">Mercado</Link>
      </nav>

      <div className="mb-5 flex items-center justify-between md:hidden">
        <Button asChild variant="outline" size="icon-sm">
          <Link to="/marketplace" aria-label="Voltar ao mercado">&lsaquo;</Link>
        </Button>
      ...
      <section className="grid gap-7 lg:grid-cols-[minmax(0,1.05fr)_minmax(22rem,0.95fr)] lg:gap-10">
        <div className="grid gap-4 md:grid-cols-[4.5rem_minmax(0,1fr)]">
          <div className="order-2 flex gap-3 overflow-x-auto md:order-1 md:flex-col" aria-label="Galeria do NFT">
            {nft.gallery.map((image, index) => (
              ...
              >
                <img src={image.url} alt="" width={image.width} height={image.height} className="size-full object-cover" />
              </button>
            ))}
          </div>
        ...
        <div className="rounded-panel bg-card p-6 lg:bg-transparent lg:p-0">
          <div className="flex items-start justify-between gap-4">
            <div>
              <Typography as="h1" variant="title">{nft.name}</Typography>
          ...
          <p className="mt-3 text-sm text-foreground" aria-label={`${nft.rating} de 5 estrelas, ${nft.reviewCount} avalia\u00e7\u00f5es`}>
            <span className="text-primary">{'\u2605\u2605\u2605\u2605\u2605'}</span> {nft.rating} ({nft.reviewCount})
          </p>

          <div className="mt-6 border-t border-border/70 pt-5">
            <Typography as="h2" variant="subheading">Sobre este NFT:</Typography>
            <Typography tone="muted" className="mt-2">{nft.description}</Typography>
          </div>
          ...
          <dl className="mt-5 space-y-2 text-sm text-muted-foreground">
            <div className="flex gap-2"><dt>ID do token:</dt><dd>{nft.tokenId}</dd></div>
            <div className="flex gap-2"><dt>Cole\u00e7\u00e3o:</dt><dd>{nft.collectionName}</dd></div>
            <div className="flex gap-2"><dt>Atributos:</dt><dd>{nft.attributes.join(', ')}</dd></div>
          </dl>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-4 border-t border-border/70 pt-5">
            ...
            <Button type="button" size="icon-sm" aria-label="Diminuir quantidade" disabled={quantity <= 1} onClick={() => setQuantity((current) => Math.max(1, current - 1))}>−</Button>
            <output aria-live="polite" className="min-w-5 text-center">{quantity}</output>
            <Button type="button" size="icon-sm" aria-label="Aumentar quantidade" disabled={quantity >= maxQuantity} onClick={() => setQuantity((current) => Math.min(maxQuantity, current + 1))}>+</Button>
            <span className="ml-auto text-xs text-muted-foreground">{maxQuantity} dispon\u00edveis</span>
          </div>

          <div className="mt-6 flex gap-3">
            <Button type="button" size="lg" className="flex-1" disabled title="A compra ser\u00e1 habilitada com a integra\u00e7\u00e3o do carrinho">
              Comprar NFT
            </Button>
          </div>
        </div>
      ...
      <section className="mt-12 border-t border-border/70 pt-8 md:mt-16">
        <Typography as="h2" variant="heading" tone="accent">Detalhes do NFT</Typography>
        <Typography tone="muted" className="mt-4 max-w-5xl whitespace-pre-line">{nft.longDescription}</Typography>
        <dl className="mt-5 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
          <div><dt className="font-semibold text-foreground">Rede</dt><dd className="mt-1 capitalize">{nft.network}</dd></div>
          <div><dt className="font-semibold text-foreground">Contrato</dt><dd className="mt-1 break-all">{nft.contractAddress}</dd></div>
        </dl>
      </section>

      {relatedNfts.length > 0 && (
        <section className="mt-12 border-t border-border/70 pt-8 md:mt-16">
          <Typography as="h2" variant="heading" tone="accent">Mais desta cole\u00e7\u00e3o</Typography>
          <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
            {relatedNfts.map((item) => <NftCard key={item.id} nft={item} />)}
          </div>
        </section>
      )}
    </main>
