...
import { createOrder, fetchOrder, recoverOrder } from '@/features/orders/orders-api'
...
export const orderQueryKeys = {
  ...
  recovery: (idempotencyKey: string) =>
    [...orderQueryKeys.all, 'recovery', idempotencyKey] as const,
}

export function useCreateOrderMutation(): UseMutationResult<Order, Error, CreateOrderVariables> {
  return useMutation<Order, Error, CreateOrderVariables>({
...
export function useOrderQuery(orderId: string): UseQueryResult<Order, Error> {
  return useQuery<Order, Error>({
    ...
    queryFn: ({ signal }): Promise<Order> => fetchOrder(orderId, signal),
...
export function usePendingOrderRecoveryQuery(
  ...
  return useQuery<Order, Error>({
    queryKey: orderQueryKeys.recovery(idempotencyKey ?? 'none'),
    ...
    queryFn: ({ signal }): Promise<Order> => {
      ...
      return recoverOrder(idempotencyKey, signal)
    },
