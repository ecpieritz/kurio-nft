...
const walletProviderLabels: Record<
  string,
  string
> = {
  metamask: 'MetaMask',
...
function DeclinedOrderCard({
  ...
  return (
    <section className="w-full max-w-xl rounded-panel border border-destructive/50 bg-card p-8 text-center shadow-card">
      ...
      >
        {reason ??
          'A transação não foi confirmada pela carteira selecionada.'}
      </Typography>
...
export function OrderPage() {
  ...
  if (
    orderQuery.error instanceof
      ApiClientError &&
    orderQuery.error.code ===
      'NOT_FOUND'
  ) {
    return (
      <main
