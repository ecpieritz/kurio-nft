...
export function useCatalogQuery(request: NftListRequest): UseQueryResult<NftListResponse, Error> {
  return useQuery<NftListResponse, Error>({
    queryKey: catalogQueryKeys.list(request),

    queryFn: ({ signal }): Promise<NftListResponse> => fetchNfts(request, signal),
...
export function useNftDetailsQuery(nftId: string): UseQueryResult<NftDetails, Error> {
  return useQuery<NftDetails, Error>({
    queryKey: catalogQueryKeys.detail(nftId),

    queryFn: ({ signal }): Promise<NftDetails> => fetchNftDetails(nftId, signal),
