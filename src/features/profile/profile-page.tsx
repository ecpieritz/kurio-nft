...
import type { CollectorProfile, UpdateProfileRequest } from '@/lib/api/contracts'
...
function getFieldError(error: Error | null, field: string): string | null {
...
function getProfileErrorMessage(error: Error | null): string | null {
  if (!error) {
    return null
  }
...
function getAvatarErrorMessage(error: Error | null): string | null {
  if (!error) {
    return null
  }
...
function getPasswordErrorMessage(error: Error | null): string | null {
  if (!error) {
    return null
  }

  if (error instanceof ApiClientError) {
    const currentPasswordError = error.fieldErrors?.find(
      (fieldError) => fieldError.field === 'currentPassword',
    )?.message
...
function validateNewPassword(values: PasswordFormValues): string | null {
...
function AccountSidebar() {
  ...
  return (
    <aside className="hidden h-fit overflow-hidden rounded-panel bg-card lg:sticky lg:top-24 lg:block">
      <Typography as="h2" variant="heading" className="px-4 pb-2 pt-5">
        Meu perfil
      </Typography>

      <nav aria-label="Configurações do perfil" className="grid text-sm">
        <Link
          to="/profile"
          activeOptions={{
            exact: true,
        ...
        <span className="flex items-center gap-3 border-l-4 border-transparent px-4 py-3 text-primary/80">
          <Icon name="danger" className="size-4" />
          Suporte
        </span>
      </nav>
...
function ProfileEditor({ profile }: ProfileEditorProps) {
  ...
  const displayNameError = getFieldError(updateProfileMutation.error, 'displayName')

  const usernameError = getFieldError(updateProfileMutation.error, 'username')

  const emailError = getFieldError(updateProfileMutation.error, 'email')

  const ensNameError = getFieldError(updateProfileMutation.error, 'ensName')

  const walletNicknameError = getFieldError(updateProfileMutation.error, 'walletNickname')

  const profileErrorMessage = getProfileErrorMessage(updateProfileMutation.error)

  const avatarErrorMessage = avatarClientError ?? getAvatarErrorMessage(updateAvatarMutation.error)
  ...
  function handleProfileSubmit(event: FormEvent<HTMLFormElement>): void {
    ...
    const request: UpdateProfileRequest = {
      ...
      walletNickname: form.walletNickname.trim(),
  ...
  function handleAvatarSelection(event: ChangeEvent<HTMLInputElement>): void {
    ...
    if (!acceptedAvatarTypes.has(file.type)) {
      setAvatarClientError('Use uma imagem PNG, JPEG ou WebP.')

      return
    }

    if (file.size > MAX_AVATAR_SIZE_BYTES) {
      setAvatarClientError('A imagem deve ter no máximo 2 MB.')
    ...
    void fileToDataUrl(file)
      ...
      .catch((error: unknown) => {
        setAvatarClientError(
          error instanceof Error ? error.message : 'Não foi possível ler a imagem selecionada.',
        )
      })
  }
      ...
      <form onSubmit={handleProfileSubmit} className="mt-6">
        <div className="grid gap-x-7 gap-y-5 md:grid-cols-2">
          <label className="grid gap-2 text-sm">
            ...
            {displayNameError && (
              <span className="text-xs text-destructive">{displayNameError}</span>
            )}
          </label>
          ...
          <label className="grid gap-2 text-sm">
            ...
            {usernameError && <span className="text-xs text-destructive">{usernameError}</span>}
          </label>
          ...
          <label className="grid gap-2 text-sm">
            ...
            <Input
              ...
              value={form.ensName}
              aria-invalid={Boolean(ensNameError)}
              onChange={(event) => {
                setProfileSaved(false)
            ...
            {ensNameError && <span className="text-xs text-destructive">{ensNameError}</span>}
          </label>
          ...
          <label className="grid gap-2 text-sm">
            ...
            <Input
              ...
              aria-invalid={Boolean(walletNicknameError)}
              onChange={(event) => {
                setProfileSaved(false)
                ...
                setForm((current) => ({
                  ...
                  walletNickname: event.target.value,
                }))
              }}
            />

            {walletNicknameError && (
              <span className="text-xs text-destructive">{walletNicknameError}</span>
            )}
          </label>
          ...
          <div className="grid gap-2 text-sm">
            ...
            <div className="flex min-h-12 flex-wrap items-center gap-3">
              ...
              >
                {updateAvatarMutation.isPending ? 'Atualizando...' : 'Alterar'}
              </Button>

              <Button
                type="button"
                variant="ghost"
                disabled={!profile.avatarUrl || updateAvatarMutation.isPending}
            ...
            {avatarSaved && !avatarErrorMessage && (
        ...
        {profileSaved && !profileErrorMessage && (
...
function PasswordEditor() {
  ...
  const passwordErrorMessage = clientError ?? getPasswordErrorMessage(passwordMutation.error)
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>): void {
    ...
    const validationError = validateNewPassword(form)
  ...
  return (
    <section className="mt-10 border-t border-border/70 pt-8">
      ...
      <form onSubmit={handleSubmit} className="mt-6 max-w-[27rem] space-y-5">
        <PasswordInput
          id="current-password"
          label="Senha atual"
        ...
        {passwordSaved && !passwordErrorMessage && (
        ...
        <Button type="submit" disabled={passwordMutation.isPending}>
          {passwordMutation.isPending ? 'Alterando...' : 'Salvar senha'}
        </Button>
      </form>
    </section>
  )
  ...
  return (
    ...
    >
      <div className="grid gap-8 lg:grid-cols-[19rem_minmax(0,1fr)]">
        ...
        <div>
          <ProfileEditor profile={profileQuery.data} />
