...
function isProvider(value: unknown): value is WalletProvider {
  return value === 'metamask' || value === 'walletconnect' || value === 'coinbase'
}

function parseSaveWalletRequest(value: unknown): SaveWalletRequest | undefined {
  if (
    !isRecord(value) ||
  ...
  if (value.ensName !== undefined && typeof value.ensName !== 'string') {
    return undefined
  }
...
function validateWalletRequest(request: SaveWalletRequest): HttpResponse<ApiErrorResponse> | null {
  if (
    ...
    return errorResponse('VALIDATION_ERROR', 'Preencha os campos obrigatórios da carteira.', 422)
  }
...
function applyPrimarySelection(
  ...
  const hasAnotherPrimary = wallets.some(
    (wallet) => wallet.id !== selectedWalletId && wallet.primary,
  )

  const shouldBePrimary = requestedPrimary || !hasAnotherPrimary

  if (shouldBePrimary) {
    wallets.forEach((wallet) => {
      wallet.primary = wallet.id === selectedWalletId
    })
  }
...
export const walletHandlers = [
  http.get(
    ...
    async ({ request }) => {
      const scenarioResponse = await applyNetworkScenario('wallets')
      ...
      const response: WalletCollection = {
        items: structuredClone(authorization.state.walletsByUser[authorization.userId] ?? []),
      }
  ...
  http.post(
    ...
    async ({ request }) => {
      ...
      const payload = parseSaveWalletRequest(body)

      if (!payload) {
        return errorResponse('VALIDATION_ERROR', 'Os dados da carteira são inválidos.', 422)
      }

      const validationResponse = validateWalletRequest(payload)

      if (validationResponse) {
        return validationResponse
      }

      const wallets = authorization.state.walletsByUser[authorization.userId] ?? []

      const now = new Date().toISOString()

      const wallet: CollectorWallet = {
        ...
        ensName: payload.ensName?.trim() || null,
      ...
      wallet.primary = applyPrimarySelection(
        wallets,
        wallet.id,
        payload.primary || wallets.length === 1,
      )

      authorization.state.walletsByUser[authorization.userId] = wallets

      authorization.state.revision += 1

      mockDatabase.write(authorization.state)

      return HttpResponse.json(structuredClone(wallet), {
        status: 201,
      })
    },
  ),
  ...
  http.patch(
    ...
    async ({ request, params }) => {
      ...
      if (!walletId) {
        return errorResponse('VALIDATION_ERROR', 'A carteira informada é inválida.', 422)
      }
      ...
      if (payload.expectedVersion !== undefined && payload.expectedVersion !== wallet.version) {
        return errorResponse('CONFLICT', 'A carteira foi atualizada por outra operação.', 409, {
      ...
      wallet.nickname = payload.nickname.trim()

      wallet.profileName = payload.profileName.trim()

      wallet.email = payload.email.trim()

      wallet.address = payload.address.trim()

      wallet.ensName = payload.ensName?.trim() || null

      wallet.referralCode = payload.referralCode.trim()

      wallet.network = payload.network

      wallet.provider = payload.provider

      wallet.primary = applyPrimarySelection(wallets, wallet.id, payload.primary)
